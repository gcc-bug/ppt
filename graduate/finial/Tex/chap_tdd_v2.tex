...
\end{algorithmic}
\end{algorithm}
算法\ref{alg-basis_dec}的正确性证明如\ref{pf-alg-basis}所示。算法\ref{alg-basis_dec}的递归次数不超过投影算子$P$的维度。因此对于$n$比特的投影算子$P$，算法\ref{alg-basis_dec}的时间复杂度为$O(n)$。
\begin{proof}
    \label{pf-alg-basis}
    首先对于TDD形式投影算子$P$中的非零路径所表示的状态\(\ket{u_i}\)，均有
\end{proof}
...
\end{algorithmic}
\end{algorithm}
算法\ref{alg-image}的正确性证明如\ref{pf-alg-image}所示。算法\ref{alg-image}的递归次数不超过投影算子$P$的维度。因此对于$n$比特的投影算子$P$，算法\ref{alg-image}的时间复杂度为$O(n)$。
\begin{proof}
    \label{pf-alg-image}
    首先对于TDD形式投影算子$P$中的非零路径所表示的状态\(\ket{u_i}\)，均有
\end{proof}

...
\end{enumerate}
% 在此基础上，可以计算该量子迁移系统$(\mathcal{H},S,\Sigma,\mathcal{T})$的可达空间。具体方法如算法\ref{alg-reachable}所示。算法\ref{alg-reachable}的正确性有定理\ref{th-markov}保证。
% \begin{theorem}\citep{2021}
%     \label{th-markov}
%     设 $d = \dim{\mathcal{H}}$。对于任何状态 $\rho$ 在 $\mathcal{H}$ 中，有：
%     \begin{align}
%     R_c(\rho) = \bigvee_{i=0}^{d-1} \text{supp} (E^i(\rho)) = \text{supp} \left(\sum_{i=0}^{d-1} E^i (\rho)\right)
%     \end{align}
%     其中 $E^i$ 是 $E$ 的第 $i$ 次幂，即 $E^0 = I$ （$\mathcal{H}$ 中的恒等操作）且 $E^{i+1} = E \circ E^i$ 对于所有 $i \geq 0$。
% \end{theorem}

...
本文工作将重点探讨采用张量决策图(Tensor Decision Diagrams， 或TDD)作为数据结构，从而缓解资源消耗的量子模型检测方法。
...
\begin{enumerate}
    ...
    \item 针对量子模型检测，设计实现了需要的TDD软件并设计数值实验，验证了这些TDD在一步迁移算法中的可行性，以及优化方案的执行效率提升。从而验证了TDD在量子模型检测中的实用性。
\end{enumerate}
% 主要工作:
...
第三章为 \textbf{在量子模型检测中应用TDD表示}，该章节主要是本文的方法论介绍。
在数据结构TDD的基础上，
详细阐述了基于TDD的量子模型检测方法，包括基于TDD表示的子空间算法、基于TDD的量子一步迁移算法和针对量子模型检测的优化技术，最后还介绍了相对应的软件系统设计。
在该章节中首先简单介绍了TDD的定义，接着回顾了量子迁移系统，然后讨论了几种比较重要的量子模型检测中的算法。
首先是对TDD表示的子空间进行基分解的算法，然后是基于TDD表示的子空间的并的算法。根据TDD表示的子空间算法，讨论了量子一步迁移算法。
之后，讨论了针对量子模型检测的问题，对TDD表示的一些优化，其中包括addition、contraction两种电路拆分方法，以及基于窗函数和用子空间近似两种基于TDD结构的优化方法。
该章的最后，简单介绍了TDD的软件系统的模块内容。

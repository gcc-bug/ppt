...
为应对这一挑战，可以引入更高效的数据结构，以较少的资源表示量子状态和量子线路，并计算其最终结果。特别地，TDD为量子电路提供了一种紧凑的表示形式，它便于实现张量网络的各种操作，这对模拟量子物理系统至关重要。如图 \ref{fig:P} 所示，通过比较矩阵形式和TDD形式的表示，可以明显看出TDD结构的紧凑性，其中TDD中实线代表高边，虚线代表低边。
...
TDD尤其适合于实施可达性分析和模型检查算法。这得益于在基于BDD（二进制决策图）的模型检查算法中使用的许多优化技术，可推广至TDD用于收缩量子电路的张量网络\citep{Chaki_2018}。该技术进步为解决量子模型检测的资源爆炸问题提供了新的可能性。
...
Simon J. Gay等人还开发了用于量子通信协议的模型检测工具\citep{Gay,gay2010specification}，但其应用限于必须建模为量子电路的协议，才能在Stabilizer code\citep{gottesman1997stabilizer}中形式表示。Ebrahim Ardeshir-Larijani等人则将此技术扩展到Stabilizer code之外，用于检测量子协议的等效性\citep{ardeshir2013equivalence,ardeshir2014verification}。
...
过去量子模型检测研究追求的是普遍性，即只针对检测量子系统的一般可达性和时间逻辑属性。未来量子模型检测的最重要目标是寻找一类更简单易于检测的属性。这是因为当前量子模型检测的执行效率非常低，且仅适用于非常小规模和深度较小的量子电路。因此，需要确定一类更简单易于检测的属性，以便当前的量子模型检测工具可以高效地进行检测\citep{ying2021model}。
...
本文工作的主要目的是借助最新的数据结构TDD实现更少资源的量子模型检测，即在保证自动化验证的前提下，利用TDD更快地验证量子系统的状态是否满足给定的属性。

通过借助新的数据结构TDD，可以更方便的表示量子状态以及量子线路，并计算最终的结果。同时TDD特别适用于实现可达性分析和模型检查算法。这是因为基于BDD的模型检查算法中使用的许多优化技术可以推广到收缩量子电路张量网络上\citep{Chaki_2018}。这些为应用TDD解决量子模型检测问题提供了可能的方案。
同时如今量子计算存在大量的验证需求。
...
在量子计算的实际研发过程中着重要作用。
TDD作为针对量子计算设计的高效数据结构，已经展示了其在表现量子电路上的优势。因此本文希望将这一数据结构应用在量子模型检测，降低量子模型检测的资源负担，从而应用到更多量子系统的验证问题中。
\section{论文结构概述}
% \begin{itemize}
...
\begin{enumerate}
    ...
    \item 
    在TDD表示子空间的基础上，提出了基于TDD的量子迁移系统的一步迁移算法。为量子模型检测提供了算法基础。同时也设计了基于电路分割以及TDD结构的优化方案。
    \item 针对量子模型检测，设计实现了需要的TDD软件。在此基础上设计了数值实验，验证了TDD在一步迁移算法中的可行性，以及优化方案的执行效率提升。从而验证了TDD在量子模型检测中的实用性。
\end{enumerate}
% 主要工作:

% 研究了基于张量网络表示量子电路和量子系统的方法，并引入了TDD(Tensor Decision Diagrams)数据结构。
% 设计了基于TDD的算法来计算量子迁移系统的可达空间，用于量子模型检测中的可达性分析。
% 提出了多种优化策略，如电路拆分、TDD结构优化和子空间近似表示等，以降低可达性分析过程中的时间和空间资源消耗。
% 进行了实验评估，证明了所提优化方法的有效性，尤其是基于contraction的电路拆分算法能显著提升效率。
...
在该章节中首先简单介绍了TDD的定义，接着回顾了量子迁移系统，然后设计了几种比较重要的量子模型检测中的算法。

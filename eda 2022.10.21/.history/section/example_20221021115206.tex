...
\begin{frame}{initial state \footfullcite{initial}}
  \begin{itemize}target:
    ...
    \end{align}
    \begin{align}
      G(p)|0\rangle = \sqrt{p(\bar{x}_i)}|0\rangle+\sqrt{1-p(\bar{x}_i)}|1\rangle
    \end{align}
    % \begin{align}
    %   G\left(p\left(x_{i}\right)\right) = R_{y}\left(2 \cos ^{-1}\left(\sqrt{p\left(\bar{x}_{i}\right)}\right)\right)
    % \end{align}
  \end{itemize}
\end{frame}

...
\begin{frame}{XAG}
  \begin{algorithm}[H]
    ...
    \begin{algorithmic}
      ...
      \STATE $\operatorname{CNOT}\left(x, t_{1}\right)$  for all  $x \in L_{1} \backslash\left\{t_{1}\right\}$
      \STATE $\operatorname{CNOT}\left(x, t_{2}\right)$  for all  $x \in L_{2} \backslash\left\{t_{2}\right\}$
      \IF{p}
      \STATE$\operatorname{NOT}\left(t_{1}\right)$
      \ENDIF
      \IF{q} 
      \STATE $\operatorname{NOT}\left(t_{2}\right)$
      \ENDIF 

...
\begin{frame}{XAG}
  \begin{algorithm} 
    \caption{Easily\_Decompose($\rho$)} 
    \label{alg-be} 
    \begin{algorithmic}[1]
      \REQUIRE a state $\rho$ in tdd
      \ENSURE a set of bases of $supp(\rho)$
      \STATE $B\gets \{\}$
      \IF{$\rho$ = EMPTY}
      \RETURN $B$
      \ELSE
      \STATE $Way \gets NotZero(\rho) $
      \STATE $P \gets SPAN(Way)$
      \STATE $B \gets B \cup P \cup Decompose(\rho - P \rho P)$
      \ENDIF
      \RETURN $B$ 
    \end{algorithmic} 
  \end{algorithm}

...
\begin{frame}{XAG}
  \begin{algorithm}[H]
    ...
    \begin{algorithmic}
      ...
      \REQUIRE  set  $p \leftarrow p(i), q \leftarrow q(i), j \leftarrow j(i), k \leftarrow k(i) $
      \REQUIRE set  $L_{1} \leftarrow \operatorname{ltfi}\left(x_{j}\right), L_{2} \leftarrow \operatorname{ltfi}\left(x_{k}\right) $
      \IF{$L_{1} \subseteq L_{2}$}
      \REQUIRE swap $L_1 \leftrightarrow L_2$ and $p \leftrightarrow q$
      \ENDIF
      \REQUIRE let  $t_{1}$  be some element in  $L_{1}\backslashL_{2}$
      \REQUIRE let  $t_{2}$  be some element in  $L_{2}$
      \REQUIRE CNOT  $\left(x, t_{1}\right)$  for all  $x \in L_{1} \backslash\left\{t_{1}\right\}$
      \REQUIRE CNOT  $\left(x, t_{2}\right)$  for all  $x \in L_{2} \backslash\left\{t_{2}\right\}$
      \REQUIRE if  p  then  \operatorname{NOT}\left(t_{1}\right) 
      \REQUIRE if  q  then  \operatorname{NOT}\left(t_{2}\right) 
      \REQUIRE $\operatorname{TOFFOLI}\left(t_{1}, t_{2}, x_{i}\right) $
